...
public class CRCUtil {
    ...
    public static byte[] hexStringToByte(String hex) {
        ...
        Log.e("数据打印", "Bytes = " + Arrays.toString(result));
        Log.e("数据打印", "string=" + bytesToHexString(result));
        return result;
    }
     ...
     * 校验和
     *
     * @param sendStr sendStr
     * @return 单字节返回
     */
    public static String checkSum(String sendStr) {
        int res = 0;
        for (int i = 0; i < sendStr.length() / 2; i++) {
            res = res + Integer.parseInt(sendStr.substring(i * 2, i * 2 + 2), 16);
        }
        //(超过0xFF，减去0x100)
        if (res > 0xFF) {
            res -= 0x100;
        ...
        return result.substring(result.length() - 2, result.length()).toUpperCase();
    }
     ...
     *
     * @param data 字符串
     * @return 异或 0x33 返回 字符串
     */
    public static String ExclusiveOrOperation(String data) {
        StringBuilder stringBuilder = new StringBuilder();
        for (int i = 0, k = data.length() / 2; i < k; i++) {
            //进行异或运算 0x33
            int num = Integer.parseInt(data.substring(i * 2, i * 2 + 2), 16);
            num = num ^ 0x33;
